...
def extract_nouns_prop(doc, list_words):
    nouns_prop = []
    for token in doc:
        #Condition for common nouns that are modified by propositional predicates
...
for token in en_doc:
    ...
    if token.pos_ == "ADJ" and token.lemma_ in en_propositional_predicates and token.head.dep_ == "ROOT":
...
for token in en_doc:
    ...
    if token.pos_ == "ADJ" and token.lemma_ in en_eventive_predicates and token.head.dep_ == "ROOT":
        children = [child for child in token.head.children]
        for child in children:
            if child.pos_ == "NOUN" and child.dep_ == "nsubj":
                en_nouns_prop.append(child.lemma_)
...
def en_my_filtering_function(pair):
    ...
    wanted_keys = ['power', 'pledge', 'voice', 'hope', 'idea', 'word', 'thrill', 'food', 'thought', 'evidence', 'opinion', 'belief', 'succession', 'remark', 'understanding', 'feeling', 'thing', 'chance', 'fight']
    key, value = pair
...
def es_my_filtering_function(pair):
    #We define the words that we want to maintain in each frequency list
    wanted_keys = ['vez', 'dueño', 'gloria', 'dicha', 'conquista', 'señorito', 'soplo', 'palabra', 'situación', 'campo', 'noticia', 'tiniebla', 'cavilación', 'instante', 'cuerpo', 'recurso', 'hombre', 'arma', 'ciencia', 'madre', 'alma', 'saber', 'personaje', 'peligro', 'sitio', 'gobierno', 'vida', 'ciudad', 'protesta', 'estado', 'señora', 'oración', 'lujo', 'mirada', 'don', 'mano', 'persona', 'cosa', 'guisa', 'fenómeno', 'mujer', 'ilusión', 'número', 'necesidad', 'riqueza', 'obra', 'cuestión', 'teniente', 'sentimiento', 'señor', 'familia', 'momento', 'vista', 'resto', 'temor', 'juez', 'amigo', 'expresión', 'importancia', 'condición', 'talento', 'acto', 'amor', 'año', 'espacio', 'casa', 'voz', 'matiz']
    key, value = pair
    if key in wanted_keys:
        #Keep the pair in the list
        return True
